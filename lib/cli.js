...
var ChallengeListing    = require('./Values/ChallengeListing'); 
...
function list (language) {
  var deferred = Q.defer();
  var challengeListing = new ChallengeListing();
  var languageLookup = defaults.supportedLanguagesArray.indexOf(language) != -1 ? [language] : defaults.supportedLanguagesArray;
  var languageDir = process.cwd();
  
  languageLookup.forEach(function (language, langIndex) {
    languageDir = path.join(languageDir, language);
    
    fs.readdir(languageDir, function (err, challenges) {
      if (err && err.code === 'ENOENT') {
        return langIndex === (languageLookup.length - 1) ? deferred.resolve(challengeListing) : null;
      }
      
      if (err) { return deferred.reject(err); }
      
      challenges.forEach(function (challenge) {
        challengeListing.addChallenge(new Challenge(challenge, language));
      });
      
      if (langIndex === (languageLookup.length - 1)) {
        deferred.resolve(challengeListing);
      }
    });
  });
